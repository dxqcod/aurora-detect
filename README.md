...
- Использована очень простая, но рабочая система IPC (межпроцессного взаимодействия). Реализован "буфера обмена" через обмен сообщениями через файлы. Отличная идея для маленькой распределённой системы.
